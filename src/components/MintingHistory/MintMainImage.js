...
const MintMainImage = () => {
  ...
  let UserID_Six = localStorage.getItem("uid_output_six");
  console.log("UserID_Six",user);
  ...
  const referral_API = async () => {
    try {
      ...
      console.log("Res_API data", responce.data.data.recordset);
      ...
      if (responce == undefined) {
        ...
        responce.forEach((item, index) => {
          arr.push({
            name: item.fname,
            id: item.uid,
            registration_date: String(item?.rdate).substring(0, 12),
            ...
            Sponsor: item.sid,
            SponsorName: item.sname,
            date: item?.packageName,
            country: item.country,
            today_left_business: item.tlamount,
            today_right_business: item.tramount,
            left_carry_forward: item.lamount,
            right_carry_forward: item.ramount,
            rank: item.rank,
          });
        });
        ...
        <div className="MintingHeader">
          <div className="MintingHeader_left">
            ...
            <input
              type="text"
              className="idinput"
              defaultValue={Idnumer}
              onChange={(e) => setgetValue(e.target.value)}
            />
            <button
              className="mintsibmit"
            ...
            <button className="mintsibmit" onClick={popoutvalue}>
              Back
            </button>
