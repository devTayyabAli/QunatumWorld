...
function ConvertWalletFundToActivationFund() {
  ...
  const CouponGenerate = async () => {
    try {
      if (parseFloat(larbalance) < parseFloat(amount)) {
        alert("Insufficial Balance.");
        return;
      }
      ...
      res = res.data.data[0];
      //if (res.address == acc) {
      let responce = await API?.post("/ConvertWalletToActivationFund", {
        user_id: uId,
        amount: amount,
      });
      ...
      if (responce == "Successfull") {
        alert("Fund Converted Successfull.");
  ...
  const dashboard_coupon = async () => {
    try {
      ...
      responce = responce.data.data;
      setLarBalance(responce[0].netbal);
  ...
  const referral_API = async () => {
    try {
      let responce = await API?.post("/ConvertWalletToActivationFundHistory", {
        uid: user,
      });
      responce = responce.data.data[0];
      // setCoupon(responce[0].coupon);
      ...
      <Layout>
        <BgLayout>
          <div className="BgLayout_Header">
            <h6>Convert Wallet Fund To Activation Fund</h6>
          </div>
          ...
          <div className="Share_tableMain">
            <div>
              <div className="col-md-4 coupn_lar lar_inputWrper">
                <label htmlFor="transfer">Available Wallet Fund Balance</label>
                <input
                  type="number"
                  name="transfer"
                  readonly=""
          ...
          <div className="BgLayout_Header">
            <h6>Converted Wallet Fund To Activation Fund History</h6>
          </div>
          <div className="Share_tableMain">
...
export default ConvertWalletFundToActivationFund;
