...
function Transfer_Coupon() {
  ...
  const CouponGenerate = async () => {
    try {
      ...
      //if (res.address == acc) {
      let responce = await API?.post("/Transfer_RapidAmount", {
        touid: topid,
        user_id: uId,
        amount: amount,
      });
  ...
  const dashboard_coupon = async () => {
    try {
      ...
      responce = responce.data.data;
      setLarBalance(responce[0].rapidFund);
  ...
  const referral_API = async () => {
    try {
      let responce = await API?.post("/rapidfundtransferHistory", {
        uid: user
      });
      responce = responce.data.data[0];
      // setCoupon(responce[0].coupon);
    ...
    <div>
      <Layout>
        <BgLayout>
          ...
          <div className="Share_tableMain">
            <div>
              <div className="col-md-4 coupn_lar lar_inputWrper">
                <label htmlFor="transfer">Available Rapid Fund Balance</label>
                <input
                  type="number"
                  name="transfer"
                  readonly=""
          ...
          <div className="BgLayout_Header">
            <h6>Rapid Fund Transfer History</h6>
          </div>
          <div className="Share_tableMain">
